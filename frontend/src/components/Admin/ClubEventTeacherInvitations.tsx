...
interface ClubEventTeacherInvitationsProps {
  eventId: string;
  onInvitationStatusChange?: () => void;
}

export function ClubEventTeacherInvitations({ eventId, onInvitationStatusChange }: ClubEventTeacherInvitationsProps) {
  ...
  const loadInvitations = async () => {
    try {
      setLoading(true);
      const data = await clubDashboardApi.getEventTeacherInvitations(eventId);
      setInvitations(data || []);
    } catch (error) {
  ...
  const handleCancel = async (invitationId: string) => {
    ...
    try {
      ...
      await clubDashboardApi.cancelEventInvitation(eventId, invitationId);
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        <h3 className="text-lg font-semibold">Invitations Envoyées aux Enseignants</h3>
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold">Invitations Envoyées aux Enseignants ({invitations.length})</h3>

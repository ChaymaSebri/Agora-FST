...
async function getMyProfile(req, res, next) {
  try {
    const user = await Utilisateur.findById(req.user._id).populate('clubId', 'nom description specialite');
    if (!user) {
...
async function updateMyProfile(req, res, next) {
  try {
    ...
    const updatedUser = await Utilisateur.findById(user._id).populate('clubId', 'nom description specialite');
